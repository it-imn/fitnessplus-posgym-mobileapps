...
} from "../../services/personal_trainer";
import { showMessage } from "react-native-flash-message";
import { ArrowWhite, ArrowBlack, IconDocument } from "../../assets";
...
const PackagePTAgreement = ({
  ...
  const getPackagePTAgreementDownload = async () => {
    try {
      const { data } = await fetchContractAgreementDownload();
      if (data) {
        setDownload(data.result);
      }
    } catch (error: any) {
      showMessage({
        message: error.message || "An error occurred",
    ...
    try {
      ...
      ReactNativeBlobUtil.config({
        ...
        addAndroidDownloads: {
          ...
          notification: true,
          title: 'Package PT Agreement downloaded successfully! Click to view',
          description: 'Package PT Agreement',
  ...
  useEffect(() => {
    Promise.all([getPackagePTAgreement(), getPackagePTAgreementDownload()]);
  }, []);
